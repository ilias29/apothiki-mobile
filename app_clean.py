...
def make_stock_table(df):
    ...
    df["Προϊόν"] = df["Προϊόν"].astype(str).fillna("")
    ...
    stock = (
        df.groupby(["Barcode", "Προϊόν", "LocationId"], dropna=False)["DeltaQty"]
        .sum()
        .reset_index()
    )
...
st.caption("Καθαρή έκδοση: barcode-first, φωτογραφία/OCR, stock ανά χώρο και αναζήτηση.")
st.success("✅ Τρέχει το νέο καθαρό app_clean.py")
...
with tab_entry:
    ...
    if uploaded_file is not None:
        ...
        if detected_name:
            ...
            st.warning("Δεν αναγνωρίστηκε καθαρά προϊόν. Γράψ' το χειροκίνητα.")
    ...
    note = st.text_input(
        "Σημείωση",
        placeholder="π.χ. αρχική καταχώρηση, βραδινή αφαίρεση, απογραφή",
    )

    if st.button("💾 Αποθήκευση Κίνησης", use_container_width=True):
        if not barcode.strip():
            st.error("Βάλε barcode. Είναι η ταυτότητα του προϊόντος, όχι διακοσμητικό.")
            st.stop()
    ...
    if df.empty:
        ...
        if search:
            stock_table = stock_table[
                stock_table["Barcode"].astype(str).str.lower().str.contains(search, na=False)
                | stock_table["Προϊόν"].astype(str).str.lower().str.contains(search, na=False)
            ]
        ...
        if not negative.empty:
            st.error("⚠️ Υπάρχουν προϊόντα με αρνητικό stock.")
...
with tab_sales:
    ...
    if df.empty:
        ...
        sales = period[
            pd.to_numeric(period["DeltaQty"], errors="coerce").fillna(0) < 0
        ].copy()
        ...
        if sales.empty:
            ...
            sales["Πωλήθηκαν"] = (
                pd.to_numeric(sales["DeltaQty"], errors="coerce").fillna(0).abs()
            )

            report = (
                sales.groupby(["Barcode", "Προϊόν"])["Πωλήθηκαν"]
                .sum()
                .reset_index()
